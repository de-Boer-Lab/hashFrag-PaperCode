...
def pairwise_smith_waterman_scores():
    ...
    elif args.blast_path is not None:
        print("Computing pairwise SW scores for identified BLASTn candidate pairs.",flush=True)
        print(f"  FASTA file:         {args.fasta_path_i}",flush=True)
        print(f"  BLASTn output file: {args.blast_path}",flush=True)
        print(f"  CPUs:               {args.n_processes}",flush=True)
        print(f"  Output path:        {args.outpath}",flush=True)
        pairs = helper.pairwise_generator_from_blast_output(args.fasta_path_i,args.blast_path)
