...
class AutosomeDataProcessor(DataProcessor):
    def __init__(
        ...
        if path_to_test_data is not None:
            self.test = preprocess_df(path=path_to_test_data,
                                   seqsize=seqsize)
        else:
            self.test = None
    ...
    def prepare_train_fixed_dataloader(self):
        
        train_fixed_ds = SeqDatasetProb(
            self.train, 
            seqsize=self.seqsize,
        )
        train_fixed_dl = DataLoader(
            train_fixed_ds, 
            batch_size=self.valid_batch_size,
            num_workers=self.valid_workers,
            shuffle=self.shuffle_val
        ) 
        return train_fixed_dl
